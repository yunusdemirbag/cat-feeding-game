...
export function GameCanvas({
  ...
  useEffect(() => {
    if (currentTime <= 0 && gamePhase === "playing") endGame();
  }, [currentTime, gamePhase, endGame]);
  useEffect(() => {
    if (score > 0 && score % 8 === 0) levelUp();
  }, [score, levelUp]);

  const getAvailableCats = () =>
    cats.filter((c) => !c.currentNeed && !c.isSatisfied);
  ...
  const moveCatToNewPosition = (catId: string, clearAll: boolean) => {
    setCats((prevCats) => {
      ...
      const availablePositions = catPositions.filter((pos) => {
        ...
        return (
          Math.sqrt(
            Math.pow(pos.x - otherCat.x, 2) + Math.pow(pos.y - otherCat.y, 2),
          ) > 80
        );
      });
      const finalPositions =
        availablePositions.length > 0 ? availablePositions : catPositions;
      const newPosition =
  ...
  const generateNewRequest = (
    catToRequest: Cat,
    forceNeed?: "food" | "water",
  ) => {
    if (catToRequest.currentNeed || catToRequest.isSatisfied) return;
    const randomNeed: "food" | "water" =
      forceNeed || (Math.random() > 0.5 ? "food" : "water");
  ...
  const getCatMessage = (cat: Cat, need: "food" | "water") => {
    ...
    if (cat.id === "misa") {
      const messages =
        need === "food"
          ? [
              "Anneciğim, karnım acıktı 🥺",
              "Biraz mama alabilir miyim?",
              "Miyav.. Acıktım...",
              "Anne yemek! Lütfen?",
              "Karnım gurulduyor da...",
            ]
          : [
              "Anne, dilim damağım kurudu 💧",
              "Su verebilir misin?",
              "Susadım anneciğim...",
              "Birazcık su lütfen?",
              "Mümkünse biraz su?",
      ...
      const messages =
        need === "food"
          ? [
              "ANNE! AÇIM! YEMEK NEREDE? 😤",
              "Açlıktan öleceğim, çabuk ol!",
              "YEMEK! YEMEK! YEMEK!",
              "O mama buraya gelecek!",
              "Miyavvv! (Açım demek bu!)",
            ]
          : [
              "ANNE! SUSADIM! 💧",
              "Çöl gibi oldum, su getir!",
              "SUUUUUUU!",
              "Hemen su istiyorum! Hemen!",
              "O su kabı dolsun artık!",
            ];
      return messages[Math.floor(Math.random() * messages.length)];
    }
  };
  ...
  const showWomanReaction = (cat: Cat, need: "food" | "water") => {
    ...
    if (cat.id === "pars") {
      normalReactions = [
        ...
        "Aç mı kalmış benim aslan oğlum?",
        "Oğlum, Parsss, yapma annem!",
        `Yine mi acıktın yakışıklım?`,
        "Pars! Sabret, geliyor!",
        "Geliyor benim kuzumun yemeği.",
      ];
    } else if (cat.id === "misa") {
      normalReactions = [
        "Mişa, kızımmm, tamam.",
        "Geliyor prensesimin maması.",
        `Güzel kızım benim, susadın mı?`,
        "Hemen bakıyorum Mişa'ma.",
        "Tabii ki bebeğim, hemen.",
      ];
    }
    const complaintReactions = [
      "Of yine mi acıktınız!",
      "Daha yeni yemedin mi sen?",
      "Biraz da kendiniz alın şuradan!",
      "Afra anne yoruldu ama!",
      "Yetişemiyorum size yahu!",
      "Bu kedilerin midesi dipsiz kuyu!",
    ];
    const reactionList =
      Math.random() < 0.8 ? normalReactions : complaintReactions;
  ...
  const handleCorrectDelivery = (cat: Cat) => {
    ...
    const happyReactions =
      cat.id === "misa"
        ? ["Teşekkürler anne! ❤️", "Harikaydı!"]
        : ["HEH ŞÖYLE! Mükemmel! 🎉", "Sonunda!"];
    showSpeechBubble(
      cat,
      happyReactions[Math.floor(Math.random() * happyReactions.length)],
      "#90EE90",
      null,
    );
    ...
    setTimeout(() => {
      setCats((prev) =>
        prev.map((c) => (c.id === cat.id ? { ...c, isSatisfied: false } : c)),
      );
  ...
  const handleWrongDelivery = (cat: Cat) => {
    ...
    const wrongReactions =
      cat.id === "misa"
        ? ["Ama ben onu istememiştim ki... 😔", "Yanlış oldu anneciğim..."]
        : ["BU DEĞİL! DİĞERİ! 😠", "Anne! Dikkatini ver!"];
    showSpeechBubble(
      cat,
        ...
        >
          {bubble.needType && (
            <div className="w-6 h-6 flex-shrink-0">
              <img
                src={bubble.needType === "food" ? "/mama.png" : "/su.png"}
                alt={bubble.needType}
